...
vec3 raymarch(vec2 p, float s) {
    ...
    vec3 viewDir = rayDirection(80.0, resolution.xy, gl_FragCoord.xy); // направление от камеры к пикселю на экране 
...
void main( void ) {
    ...
    vec2 p = (-resolution.xy + 2.0*gl_FragCoord.xy) / resolution.y; // точка на экране [-1;1]
